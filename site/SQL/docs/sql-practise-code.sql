...
select
 company_id , employee_id , employee_name , salary,
 max(salary) over (partition by company_id order by salary rows between unbounded preceding and unbounded following ) as max_sal_per_company
 from salaries);


 4. Countries You Can Safely Invest In
...
6. Active users : Imp -- date diff between consecutive days.
...
9. Customers Who Bought Products A and B but Not C
...
11. Number of Trusted Contacts of a Customer
